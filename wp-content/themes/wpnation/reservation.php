...
?>
		...
		<div class="container reservation-page-main-wrap">
			...
			<?php if ( $step2 ) { 
				...
				if ( ! isset( $_POST['resform_step1'] ) || ! wp_verify_nonce( $_POST['resform_step1'], 'resform' ) ) {
					_e("Sorry, your nonce did not verify. Please try to start reservation again.","nation");
					exit;
				} else {
					...
					<form id="resend-step2" method="POST">
						<div id="reservation-info">
							...
							<div id="reservation-info-content">
								<div id="reservation-check-in">
									<div><?php _e('Check-in','nation'); ?></div>
									<input type="text" value="<?php echo $_POST["check-in"] ?>" id="check-in-date" name="check-in" class="reservation-form-field" readonly="true">
								</div>
								<div id="reservation-check-out">
									<div><?php _e('Check-out','nation'); ?></div>
									<input type="text" value="<?php echo $_POST["check-out"] ?>" id="check-out-date" name="check-out" class="reservation-form-field" readonly="true">
								</div>
								<div class="clear"></div>
								...
								wp_nonce_field( 'resform', 'resform_step1' );
								
								?>
								<button id="resend-submit" type="submit"><?php _e('Apply changes','nation'); ?></button>
					...
					if ( $the_query->have_posts() ) {
						...
						?>
							...
							<form class="room-reservation-wrap" method="post">
								...
								<div class="clear"></div>
								<?php wp_nonce_field( 'resform', 'resform_step2' ); ?>
							</form>
			...
			if ( $step3 ) { 
				...
				if ( ! isset( $_POST['resform_step2'] ) || ! wp_verify_nonce( $_POST['resform_step2'], 'resform' ) ) {
					_e("Sorry, your nonce did not verify. Please try to start reservation again.","nation");
					exit;
				} else {
					...
					for ($ra=0;$ra<count($dateRange);$ra++) {
						...
						$roomAvailty[$ra] = $wpdb->get_row( $wpdb->prepare( 
							"SELECT * FROM {$wpdb->prefix}nation_booking_availability WHERE calendar_id='%d' 
							AND day = $dayCurrent AND month = $monthCurrent AND year = $yearCurrent AND availability >= %d ",
							$_POST["room-id"], $_POST["room-number"]
						) );
					...
					$_SESSION["room-number"] = $_POST["room-number"];
					$_SESSION["total"] = $total;
			
					$incTax = 0;
					if ( isset($addTax) && $addTax == 1 && isset($hideTax) && $hideTax == 0 ) {
						$incTax = round($_SESSION["total"]*$_POST["room-number"]*($tax/100),2); 
						$_SESSION["inctax"] = $incTax;
					} else {
						$_SESSION["inctax"] = "";
					}
					...
					<div id="reservation-info-content">
						...
						<div id="total-price-wrap">
							<div id="total-price"><?php _e('Total price:','nation'); ?> 
								<span id="price"><?php echo round($_SESSION["total"]*$_POST["room-number"]+$_SESSION["inctax"],2).$currencySymbol; ?></span>
							</div>
							...
							<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">							
								<div class="modal-body">
									<table>
										...
										<tr>
											<td><?php printf (__('Tax (%d%%):','nation'),$tax) ?></td>
											<td><?php echo "+".$_SESSION["inctax"].$currencySymbol; ?></td>
										</tr>
										
										<?php } ?>
										<tr>
											<td id="modal-total"><?php _e('Total:','nation'); ?> </td>
											<td id="modal-price-total"><?php echo round($_SESSION["total"]*$_POST["room-number"]+$_SESSION["inctax"],2).$currencySymbol; ?></td>
										</tr>
									</table>
								</div>
								<div class="modal-footer">
			...
			<div class="eleven columns reservation-content page-left-sidebar step3">
				<form id="step3-form" method="post">
					...
					<div id="payment-reservation-form">
						...
						<div id="resform-cvc" class="form-field-wrap">
							<div class="resform-header"><?php _e("CVC Code","nation") ?> <span class="main-reservation-form-asterisk">*</span></div>
							<input type="text" name="resform-cvc" class="form-field-wrap required-field">
						</div>
					...
					<?php } 
					wp_nonce_field( 'resform', 'resform_step3' ); 
					?>
				</form>
			</div>
			...
			if ( $step4 && isset( $_POST["payments_method"] ) && $_POST["payments_method"] == "paypal" ) {
				...
				if ( ! isset( $_POST['resform_step3'] ) || ! wp_verify_nonce( $_POST['resform_step3'], 'resform' ) ) {
					_e("Sorry, your nonce did not verify. Please try to start reservation again.","nation");
					exit;
				} else {
					...
					if ( isset($_POST["resform-coupon"]) && $_POST["resform-coupon"] != "" && $_POST["resform-coupon"] == $coupon_name ) {
						$coupon = $coupon_discount/100;						
						$discount = $_SESSION["total"]*$_SESSION["room-number"]*(float)$coupon;
						$total = $_SESSION["total"]*$_SESSION["room-number"]-$discount;
					...
					} else if ( isset($_POST["resform-coupon"]) && $_POST["resform-coupon"] != "" && $_POST["resform-coupon"] == $coupon_name2 ) {
					...
					} else if ( isset($_POST["resform-coupon"]) && $_POST["resform-coupon"] != "" && $_POST["resform-coupon"] == $coupon_name3 ) {
						...
						$total = $_SESSION["total"]*$_SESSION["room-number"];

						if ( isset($addTax) && $addTax != 0) { 
							$taxC = $total*($tax/100); 
						}
					
						$taxC = $_SESSION["inctax"]; 
					}
			...
			if ( $step4 && isset($_GET["token"]) && isset($_GET["PayerID"]) && !isset($_GET["reservation"]) ) {
				...
				if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
					...
					sendEmail('without_approval', $roomID, $lastID, $checkIn, $checkOut, $sPrice, $noItems, $noAdult, $noChildren,
						$email, $sName, $sSurname, $comments, 0, 0, 0, 0, 0, 0
					);
			...
			if ( $step4 && isset( $_POST["payments_method"] ) && $_POST["payments_method"] == "creditcard" ) {
				...
				if ( ! isset( $_POST['resform_step3'] ) || ! wp_verify_nonce( $_POST['resform_step3'], 'resform' ) ) {
					...
					$wpdb->insert($wpdb->prefix.'nation_booking_reservation', array(
						...
						'check_in' => date($rangeFormat, strtotime($_POST["check-in"])),
						'check_out' => date($rangeFormat, strtotime($_POST["check-out"])),
					...
					sendEmail('without_approval', $_POST["room-id"], $lastID, date($rangeFormat, strtotime($_POST["check-in"])),
						date($rangeFormat, strtotime($_POST["check-out"])), $total, $_POST["room-number"], json_encode($aroom), json_encode($croom),
						...
						$_POST["resform-expirationmonth"], $_POST["resform-expirationyear"], $_POST["cvc"]
					);
