...
function sendEmail ( $type, $cal_id, $reservation_id, $checkin, $checkout, $total, $items, $adult, 
	$child, $email, $name, $surname, $comments, $cardtype, $cardholder, $cardnumber, $expmonth, $expyear, $cvc ) {
	...
	if (function_exists ( 'icl_translate' )){
		...
		if ( $type == 'without_approval' ) {
			$subject = icl_translate('nation', 'without_confirmation_email_header', $bookingSettings->without_confirmation_email_header);
			$content = icl_translate('nation', 'without_confirmation_email_content', $bookingSettings->without_confirmation_email_content);
		} else if ( $type == 'approval' ) {
			$subject = icl_translate('nation', 'confirmation_email_header', $bookingSettings->confirmation_email_header);
			$content = icl_translate('nation', 'confirmation_email_content', $bookingSettings->confirmation_email_content);
		} else if ( $type == 'rejected' ) {
			$subject = icl_translate('nation', 'rejected_email_header', $bookingSettings->rejected_email_header);
			$content = icl_translate('nation', 'rejected_email_content', $bookingSettings->rejected_email_content);
		} else if ( $type == 'canceled' ) {
			$subject = icl_translate('nation', 'cancelation_email_header', $bookingSettings->cancelation_email_header);
			$content = icl_translate('nation', 'cancelation_email_content', $bookingSettings->cancelation_email_content);
		}
	...
	$email_content .= "<strong>".__("CVC code","nation").":</strong> ".$cvc."<br>";
	$email_content .= "<strong>".__("Cardholder name","nation").":</strong> ".$cardholder."<br>";
	$email_content .= "<strong>".__("Card number","nation").":</strong> XXXX-XXXX-XXXX-".substr($cardnumber,-4)."<br>";
