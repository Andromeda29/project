...
function nation_booking_add_reservation_func() {
	...
	if ( $error == 0 ) {
		...
		sendEmail ( 'without_approval', $_POST["cal_id"], $_POST["reservation_id"], $_POST["check-in"], $_POST["check-out"], $price, 
		$_POST["room-number"], $beAdults, $beChild, $_POST["email"], $_POST["name"], $_POST["surname"], 
		$_POST["comments"], $_POST["cardtype"], $_POST["cardholder"], $_POST["cardnumber"], $_POST["expmonth"], $_POST["expyear"], $_POST["cvc"] );
...
function nation_booking_edit_reservation_func() {
	...
	if ( $_POST["ispaypal"] == 0 ) {
		...
		array( 'check_in' => $_POST["reservation-checkin-edit"], 
			...
			'cvc' => $_POST["reservation-cvc-edit"]
		), 
...
function nation_booking_approve_reservation_func() {
	...
	if ( isset($_POST["reservation_id"]) ) {
		...
		$reservation->comments, $reservation->card_type, $reservation->cardholder_name, $reservation->card_number, $reservation->expiration_month, $reservation->expiration_year, $reservation->cvc );
...
function nation_booking_date_modification_func() {
	...
	if ( !isset($_POST['nation_date_modification_verify']) || !wp_verify_nonce($_POST['nation_date_modification_verify'],'nation_date_modification_verify') ) {
		...
		if ( isset($priceArray) && count($priceArray) > 1 ) {
...
function nation_booking_settings_edit_func() {
	...
	$hideTax = (isset($_POST['hide_tax']))?$_POST['hide_tax']:0;
	$addTax = (isset($_POST['add_tax']))?$_POST['add_tax']:0;	
	$enableCoupon = (isset($_POST['enable_coupon']))?$_POST['enable_coupon']:0;
	
	if ( function_exists('icl_register_string') ) {
		icl_register_string('nation', 'without_confirmation_email_header', $_POST['without_confirmation_email_header']);
		icl_register_string('nation', 'without_confirmation_email_content', $_POST['without_confirmation_email_content']);
		icl_register_string('nation', 'confirmation_email_header', $_POST['confirmation_email_header']);
		icl_register_string('nation', 'confirmation_email_content', $_POST['confirmation_email_content']);
		icl_register_string('nation', 'cancelation_email_header', $_POST['cancelation_email_header']);
		icl_register_string('nation', 'cancelation_email_content', $_POST['cancelation_email_content']);
		icl_register_string('nation', 'rejected_email_header', $_POST['rejected_email_header']);
		icl_register_string('nation', 'rejected_email_content', $_POST['rejected_email_content']);
	}
...
function nation_booking_paypal_settings_edit_func() {
	...
	$paypalEnabled = (isset($_POST['paypal_enabled']))?$_POST['paypal_enabled']:0;
	$paypalSandboxEnabled = (isset($_POST['paypal_sandbox_enabled']))?$_POST['paypal_sandbox_enabled']:0;
	
	$wpdb->update( $wpdb->prefix.'nation_booking_settings', array( 
		'paypal_enabled' => $paypalEnabled,
		'sandbox_enabled' => $paypalSandboxEnabled,
