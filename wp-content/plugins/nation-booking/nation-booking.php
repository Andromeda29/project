...
function nation_booking_install() {
	...
	$nation_reservation = "CREATE TABLE $table_name3 (
		...
		cvc TEXT COLLATE utf8_unicode_ci NOT NULL,
...
function nation_booking_show_admin() {
	...
	if (count($calendars) != 0){
		...
		_e("There's no calendars found. You can create a new calendar by using the form below.","nation");
		echo "<br>";
	}
	
	echo "</table>";
	echo "<br><hr><br><h2>".__("Create a New Calendar:","nation")."</h2>";
...
function nation_booking_show_reservations() {
	...
	echo "<h2>".__("Select a Calendar:","nation")."</h2>";
	echo "<select id='select-calendar-reservation'><option>".__("Select a Calendar","nation")."</option>";
...
function nation_booking_ajax_show_reservations() {
	...
	if ( isset ($_REQUEST['cal_id']) ) {	
				...
				foreach( $reservations as $reservation ) {
					...
					if ( $reservation->paypal_payment == 0 ) {
						echo "<td class='content-row'>
							...
							".__("cvc code:","nation")." &nbsp;<b>$reservation->cvc</b> <br>
					...
					if ( $reservation->paypal_payment == 0 ) {
						...
						echo "<div class='imitate-row'>name: <input name='reservation-name-edit' value='$reservation->name'><br>
							...
							cvc: <input name='reservation-cvc-edit' value='$reservation->cvc'><br>
		...
		<tr><td class='label'><strong>".__("CVC code","nation")."</strong></td><td><input type='text' name='cvc' id='surname' class='regular-text'></td></tr>
...
function nation_booking_add_script() {
	...
	wp_register_script('cookie', plugins_url('includes/js/nation-cookie.js', __FILE__), array('jquery', 'nation-admin-script'), false, true);
